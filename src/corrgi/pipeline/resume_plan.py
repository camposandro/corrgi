...
class CorrgiResumePlan(PipelineResumePlan):
    ...
    def _gather_plan(self, args: CorrgiArguments):
        ...
        with self.print_progress(total=4, stage_name="Planning") as step_progress:
            ...
            if self.is_reducing_done() and not self.is_mapping_done():
    ...
    def read_done_mapping_keys(self) -> list[str]:
        """Inspect the stage's directory of done files, fetching the keys from done file names."""
        stage_dir = Path(file_io.append_paths_to_pointer(self.tmp_path, self.MAPPING_STAGE))
        done_keys = []
        for directory in stage_dir.iterdir():
            if directory.is_dir():
                pixel_dir = directory.name
                for file in directory.iterdir():
                    if file.is_file() and file.name.endswith("_done"):
                        key = file.stem.removesuffix("_done")
                        done_keys.append(f"{pixel_dir}/{key}")
        return done_keys
