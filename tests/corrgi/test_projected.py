...
def test_pcf_natural_counts_are_correct(
    ...
    estimator = NaturalEstimator(
        ProjectedCorrelation(params=pcf_params), output_dir=tmp_path
    )
...
def test_pcf_natural_estimate_is_correct(
    ...
    estimate = compute_autocorrelation(
        ...
        rand_catalog_dir,
        output_dir=tmp_path,
        corr_type=ProjectedCorrelation,
        params=pcf_params,
    )
...
def test_pcf_counts_with_weights_are_correct(
    ...
    estimator = NaturalEstimator(
        ProjectedCorrelation(params=pcf_params, use_weights=True), output_dir=tmp_path
    )
    counts_dd, counts_rr, _ = estimator.compute_autocorrelation_counts(
        pcf_gals_weight_dir, pcf_rans_weight_dir
    )
...
def test_pcf_catalog_has_no_redshift(
    data_catalog_dir, rand_catalog_dir, pcf_params, tmp_path
):
    with pytest.raises(ValueError, match="ph_z does not exist"):
...
def test_pccf_counts_with_weights_are_correct(
    ...
    counts_cd, counts_cr = estimator.compute_crosscorrelation_counts(
        pcf_gals_weight_dir, pcf_gals1_weight_dir, pcf_rans_weight_dir
    )
